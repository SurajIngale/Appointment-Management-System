...
async function authenticateDoctor(req, res) {
    try {
        ...
        const token = jwt.sign({_id: doctor._id},'suraj',{expiresIn:'1h'})
        res.status(200).send({ message: "Authentication successful", accessedToken:token, doctorName: doctor.doctorName, doctorId: doctor._id  });
    } catch (error) {
